...
const connectionOptions = {
        ...
        mobile: useMobile, 
        browser: ['Nigromante', 'Chrome', 'Linux'],
        auth: {
            creds: state.creds,
    ...
    if (pairingCode && !conn.authState.creds.registered) {
        ...
        if (!!phoneNumber) {
            ...
            if (!Object.keys(PHONENUMBER_MCC).some(v => numeroTelefono.startsWith(v))) {
                console.log(chalk.bgBlack(chalk.redBright("Comience con el código de país de su número de WhatsApp, Ejemplo: +5213331199136")))
                process.exit(0)
            }
        } else {
            numeroTelefono = await question(chalk.bgBlack(chalk.greenBright(`Por favor, escriba su número de WhatsApp 😍\nPor ejemplo: 5213331199136 : `)))
...
setInterval(async () => {
  ...
  const bio = `NigromanteBot-MD 🚀 | sinombre913 | 𝖠𝖼𝗍𝗂𝗏o 𝖣𝗎𝗋𝖺𝗇𝗍𝖾: ⏳️ ${uptime}`;
